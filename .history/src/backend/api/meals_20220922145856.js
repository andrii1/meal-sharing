...
router.get("/:id", async (request, response) => {
  try {
    if (!request.params.id) {
      return response.status(403).json({ error: "No params" });
    } else {
...
router.put("/:id", async (request, response) => {
  try {
    if (!request.params.id) {
      ...
      response.status(201).json({ message: "Meal updated", id: newMeal[0] });
    }
  } catch (error) {
    throw error;
  }
  ...
  try {
    if (!request.params.id) {
      ...
      const deletedMeal = await knex("meal")
        .where({ "meal.id": request.params.id })
        .del();
      response.status(201).json({ message: "Meal deleted", id: newMeal[0] });
    }
  } catch (error) {
    throw error;
  }
