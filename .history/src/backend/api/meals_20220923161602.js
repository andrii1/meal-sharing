...
router.get("/", async (request, response) => {
  ...
  if (request.query.maxPrice) {
    ...
    let queryIncludesAllowedSort = sortAllowed.some(element => orderBy.includes(element));
    if (queryIncludesAllowedSort) {query = query.orderByRaw(orderBy);} else {
      response.404.message({"bad request"})
    }
